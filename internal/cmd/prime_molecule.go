...
func outputDeaconPatrolContext(ctx RoleContext) {
	...
	cfg := PatrolConfig{
		...
		WorkLoopSteps: []string{
			...
			"At cycle end:\n   - Normal path:\n     * Report: `" + cli.Name() + " patrol report --summary \"<brief summary of observations>\"`\n     * This closes the current patrol, starts a new cycle, then runs handoff so a fresh session continues\n   - If context HIGH before report:\n     * Send handoff: `" + cli.Name() + " handoff -y -s \"Deacon patrol\" -m \"<observations>\"`\n     * Exit cleanly (daemon respawns fresh session)",
		},
	}
